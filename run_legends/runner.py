from .router import SaharaRouter
...
from .database.engine import SaharaDbManager
from .database.models import SaharaBaseModel
...
import traceback
import asyncio
from .self_sender import SelfSender


class SaharaRunner(ModernRunner):
    def __init__(self):
        self.Router = SaharaRouter
        super().__init__()
    ...
    async def run_task(self, data):
        ...
        async with semaphore:
            async with SaharaDbManager(build_db_path(self.db_name), SaharaBaseModel) as db_manager:
                proxy = data['proxy']
                client = data['client']
                session = get_session('https://legends.saharalabs.ai',
                                      proxy.session_proxy)
